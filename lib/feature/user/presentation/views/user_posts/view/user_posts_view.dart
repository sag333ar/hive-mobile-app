...
class UserPostViewWidget extends StatelessWidget {
  const UserPostViewWidget(
      {super.key, required this.postType, required this.accountName});
  ...
  Widget build(BuildContext context) {
    final controller = context.read<UserPostController>();
    return Selector<UserPostController, ViewState>(
  ...
  Widget _dataState(UserPostController controller) {
    return ScrollEndListener(
      loadNextPage:()=> controller.loadNextPage(),
      child: Selector<UserPostController, List<PostFeedModel>>(
